...
| foreman #80 / #82: visible `--app` alias on app-scoped manifest commands | `internal/dynacmd.TestIsAppsScopedCommand`, `internal/dynacmd.TestMakeFlagNormalizer` | `isAppsScopedCommand` matches only `/apps/:id[/...]` endpoints with a primary `id` positional, so `services/postgresql/show --id <svc>` cannot silently accept `--app`. `--app` is now a real cobra flag (registered in `buildLeafCommand` on apps-scoped commands), so it shows in `--help` (#82). The normalizer no longer redirects `app` → `id`; `--App` / `--APP` still reach the registered flag via the existing camelCase→kebab lowercase. Primary `--id` redirect (e.g. `--id` → `--job-id` on `jobs/show`) and camelCase pass-through preserved. Conflict (both `--id` and `--app` set with different values) and missing-arg hint mentioning `--app` both covered by per-leaf smoke + pure-helper unit. |
| foreman #83: `--command true` (and other JSON-keyword literals) on string-typed array fields | `internal/dynacmd.TestCoerceArrayFlagValue_StringItemTypeNeverJSONCoerces`, `internal/dynacmd.TestCoerceArrayFlagValue_ObjectItemTypeStillJSONCoerces` | `coerceArrayFlagValue` short-circuits when the manifest field's `itemType == "string"`, returning the raw `[]string` verbatim so JSON keywords (`true`, `false`, `null`), numeric tokens, and JSON-shaped fragments all survive as literal strings on the wire. Conductor's `command[0] must be a non-empty string (got boolean)` 400 is the symptom the carve-out prevents. Object-element fields (`--service-port-mappings`, ...) keep the existing I25-B JSON-unwrap behaviour. |
| Objective 47 / story 80: `services harbor build-image` verb | `internal/deploy.TestExcludeFromTarball`, `internal/harbor.TestResolveUploadURL`, `internal/mcp.TestBuildServicesHarborBuildImageArgs_HappyPath`, `internal/mcp.TestBuildServicesHarborBuildImageArgs_Required`, `cmd.TestHarborBuildImageJSONResponseShape`, `cmd.TestHarborBuildImageSummary`, `cmd.TestSummarizeHarborBuildImage` | Build-context archive membership: `CreateBuildContextTarball`'s option set keeps dotfiles and `runos.*.yaml` (a generic Dockerfile may COPY them) while the app-deploy set strips them; `.dockerignore` is honored in both. Upload-URL hardening: a relative URL resolves against the trusted base; an absolute URL must be https (http/other scheme, empty, or hostless rejected) so the bearer token can't downgrade or be bounced to another host. MCP shim always forwards `--follow --yes`, strips the cid `(Name)` suffix, forwards repeatable `--tag`/`--build-arg`, and errors on missing context/repo/tags. `--json` envelope: fire-and-forget omits the result fields; `--follow` populates `durationMs` + `skippedBecauseCached` (`*bool`, explicit `false` distinguishable from absent). Terminal-state phrasing picks duration vs no-duration vs generic completion. |
