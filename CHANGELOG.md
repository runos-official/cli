...
- **`apps diff` no longer flags local-only `sourceDir` as drift forever**: the diff engine strips top-level `sourceDir:` lines from local bytes before comparison since `sourceDir` is a CLI-only field that never reaches conductor. Pulled yamls including `sourceDir: ..` (the directory-per-app shape) no longer report drift on every diff.
