...
- **`runos deploy` auto-follows in CI (non-TTY stdin)** (V10): pre-fix, the deploy step exited 0 the moment the build job was queued unless `--follow` was passed explicitly. CI workflows that copied the canned template silently exited green even when the build / rollout failed afterwards. Mirrors the `--yes` non-TTY auto-skip pattern: stdin is a TTY -> default `false`; stdin not a TTY (CI) -> default `true`. `--follow=false` is the explicit opt-out for the unusual case.
...
- **`apps pull` warns when local yaml path diverges from the server's `configPath`** (V2): pre-fix, pulling a VCS-deploy app to the canonical `runos.<cid>.<id>.yaml` naming with no awareness of where the AppDocument's `configPath` said the yaml lives would silently set up the next VCS deploy to fail at the cluster-agent's yaml-fetch step. The warning names both paths and points at the corrective `apps_update --configPath` command. Suppressed cleanly when not in a git checkout (no repo-relative path to compare).
