...
`runos services diff` now compares service settings rather than YAML text. A bare node affinity key, a comment, a different key order, or an equivalent numeric spelling reports in sync. Actual edits and pin removals still report drift. `runos services pull` still requires `--force` before overwriting any changed local text.
...
gains the failing backing service's name, type or reason, exactly that attribution was discarded in
the one place an operator scanning a list would look for it (objective 93). The three summary
patterns — `link`/`name`, `state`/`message`, `desired`/`ready`/`available` — now fire only when the
object carries NOTHING BEYOND the keys the summary spells out; anything richer falls through to the
lossless `key=value` rendering that was already there. That rendering now leads with the keys the
summaries consume — the state first, then the message — and alphabetises the rest, so that in a
narrow table column the health verdict is what you read and the extra attribution is what gets
elided, rather than the other way round. An object the summary already described in full renders
byte for byte as before, so `runos apps network-access`, the `replicas` block and a plain `state`
cell are untouched. `--json` was never affected and is unchanged.

**The claim that an enriched status survives the CLI is now checked in rather than assumed.**
`testdata/status/status-commands.json` is generated by `scripts/status_command_set.py` from a live
manifest fetch and lists every command that returns a service status read — every path ending in
`status`, plus the cluster deployment-readiness reader — with its declared output type and fields.
The tests iterate that file rather than a hand-written list, feeding each command three enriched
