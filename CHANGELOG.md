...
lossless alphabetised `key=value` rendering that was already there. An object the summary already
described in full renders byte for byte as before, so `runos apps network-access`, the `replicas`
block and a plain `state` cell are untouched. `--json` was never affected and is unchanged.
